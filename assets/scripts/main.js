...
async function getRecipes() {
  ...
  //const items = JSON.stringify(localStorage.getItem('recipes'));
  ...
  async function promiseFunc(resolve, reject) {
    for (let i = 0; i < RECIPE_URLS.length; i++) {
      try { 
        const newRecipe = await fetch(RECIPE_URLS[i]);
        const recipeJson = await newRecipe.json();
        recipes.push(recipeJson);
        if (i == RECIPE_URLS.length-1) {
          saveRecipesToStorage(recipes);
          const accept = resolve(recipes);
        }  
      } catch(error){
        console.log(error);
